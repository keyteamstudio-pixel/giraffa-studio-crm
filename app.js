...
function vCommessa() {
  ...
  if (t === "varianti") {
    h += '<div class="cardhead"><h2>Varianti e lavori extra</h2><button class="btn sm ghost" data-new="vari" data-ctx="' + k.idlField("tipo", "Tipo", sel(["Nota", "Chiamata", "Email", "Meeting"], r.tipo || "Nota")) + "</div>" +
...
function openRiga(kid, rid) {
  ...
  modal('<form class="box wide" data-riga-save="' + k + ":" + (rid || "") + '"><h2>' + (rid ? "Modifica voce" : "Nuova voce di preventivo") + "</h2>" +
    ...
    '<div class="row2">' + selField("assegnato_id", "Chi la esegue", opt(PROS_PRO(), r.assegnato_id)) + selField("stato", "Stato", sel(["Da iniziare", "In corso", "Consegnato"], r.stato  + '">+ Nuova variante</button></div>';
...
document.addEventListener("click", async function (e) {
  ...
  if (d.tstop) { await stopTimer();Ore & timesheet", "Registro ore " + (persp === "me" ? "personali" : "dello studio"), '<button class="btn sm" data-new="ore">+ Registra ore</button>');
  h += '<div class="grid g4">' +
...
function vCliente() {
  ...
  h += '<div class="grid g4">' +
    ...
    kpi(eur(sum(pg.filter(function (p) { return p.stato === "Incassato"; }), function (p) { return p.importo; })), "Incassato",  return; }
});
...
document.addEventListener("drop", async function (e) {
  var c = e.target.closest && e.target.closest(".kcol");
  if (!c |eur(sum(pg.filter(function (p) { return p.stato !== "Incassato"; }), function (p) { return p.importo; })) + " da incassare") +
